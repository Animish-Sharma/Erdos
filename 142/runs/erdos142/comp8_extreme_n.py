#!/usr/bin/env python3
"""
comp8_extreme_n.py — Extend bad_frac + min_wfrac measurements to N=15000..30000
...
Prior results (cumulative):
  N=5000:  avg_bad_frac=0.9107 avg_min_wfrac=0.1986 avg_witnesses=370
  N=7000:  avg_bad_frac=0.9233 avg_min_wfrac=0.1987 avg_witnesses=448
  N=10000: avg_bad_frac=0.9354 avg_min_wfrac=0.1984 avg_witnesses=540

Schedule for this run:
  N=15000: 4 trials
  N=20000: 3 trials
  N=30000: 2 trials (skipped if >10 min total elapsed)
"""
...
def run_N(N, trials, restarts=2, verbose=True):
    ...
    for trial in range(trials):
        ...
        results.append(r)
        # Progress update every trial at larger N
        if verbose:
...
def main():
    # Schedule: fewer trials at larger N due to computation time
    # N=30000 only if total elapsed < 600s
    schedule = [
        (15000, 4),
        (20000, 3),
        (30000, 2),
    ...
    TIME_LIMIT = 600.0  # 10 minutes total before skipping N=30000

    t_global = time.time()
    summaries = []

    print("comp8_extreme_n: bad_frac + min_wfrac at N=15000..30000", flush=True)
    print("=" * 120, flush=True)
    print("Prior data: N=5000→0.9107, N=7000→0.9233, N=10000→0.9354 (avg_bad_frac)", flush=True)
    print("=" * 120, flush=True)

    for N, trials in schedule:
        elapsed_so_far = time.time() - t_global
        if N == 30000 and elapsed_so_far > TIME_LIMIT:
            print(f"\n[N={N}] SKIPPED — time limit reached ({elapsed_so_far:.0f}s > {TIME_LIMIT:.0f}s)",
                  flush=True)
            continue

        print(f"\n[N={N}] running {trials} trials... (elapsed so far: {elapsed_so_far:.1f}s)",
              flush=True)
        t_block = time.time()

#!/usr/bin/env python3
"""
comp7_extreme_n.py — Extend bad_frac + min_wfrac measurements to N=6000..10000
    ...
    for trial in range(trials):
        ...
        if verbose:
            ...
            print(f"  trial {trial+1}/{trials}: M={r['M']} bad_frac={r['bad_frac']:.4f} "
                  f"witnesses={r['n_witnesses']} ({elapsed_trial:.1f}s)", flush=True)
...
def main():
    # N=6000 with 15 trials; N=7000 with 12; N=8000 with 10; N=10000 with 8
    schedule = [
        (6000, 15),
        (7000, 12),
        (8000, 10),
    ...
    print("comp7_extreme_n: bad_frac + min_wfrac at N=6000..10000", flush=True)
    print("=" * 120, flush=True)

    for N, trials in schedule:
        print(f"\n[N={N}] running {trials} trials...", flush=True)
        t_block = time.time()

#!/usr/bin/env python3
"""
comp12_extreme_n_bad_frac.py — Extend bad_frac + min_wfrac to N=250000
...
Prior results (trend):
  ...
  N=150000: avg_bad_frac=0.9863 avg_min_wfrac=0.200  avg_witnesses≈1600
  N=200000: avg_bad_frac=0.9890 avg_min_wfrac=0.200  avg_witnesses≈1871
  Prediction N=250000: bad_frac ≈ 0.9920 (log-model)

Schedule:
  N=250000: 1 trial (2 if time permits; estimate ~40-50 min per trial)
"""
...
def main():
    # N=250000: estimate ~40-50 min per trial (build ~25min + analysis ~20min)
    # We run 1 trial guaranteed; add 2nd if first finishes under 45 min.
    TOTAL_LIMIT = 7200.0           # 2 hours total cap for safety
    SECOND_TRIAL_THRESHOLD = 2700.0  # add 2nd trial if first < 45 min

    t_global = time.time()
    summaries = []

    print("comp12_extreme_n: bad_frac + min_wfrac at N=250000", flush=True)
    print("=" * 120, flush=True)
    print("Prior trend: N=200000→bad_frac=0.9890, avg_min_wfrac=0.200, witnesses≈1871", flush=True)
    print("Prediction:  N=250000→bad_frac≈0.9920 (log-model: +0.003 per 50k-N)", flush=True)
    print("=" * 120, flush=True)
    ...
    if r1 is not None and r1['n_witnesses'] == 0:
        print(f"\n!!! COUNTEREXAMPLE CANDIDATE: N={N} trial 1 has 0 witnesses !!!", flush=True)
        print(f"!!! bad_frac={r1['bad_frac']:.6f}, n_popular={r1['n_popular']} !!!", flush=True)

    # Optionally add a 2nd trial
    elapsed_so_far = time.time() - t_global
    if (t1 < SECOND_TRIAL_THRESHOLD and
            elapsed_so_far + t1 < TOTAL_LIMIT - 120):
        ...
        if r2 is not None:
            results.append(r2)
            if r2['n_witnesses'] == 0:
                print(f"\n!!! COUNTEREXAMPLE CANDIDATE: N={N} trial 2 has 0 witnesses !!!", flush=True)

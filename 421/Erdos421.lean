...
def modifiedGreedySmoothExceptionPredicate (n : Nat) : Prop :=
  n < 3 ∨ ∃ cert : SmoothExceptionData n, cert.smooth_bound
...
def modifiedGreedyTerminalSlabExceptionPredicate (n : Nat) : Prop :=
  ∃ cert : TerminalSlabExceptionData n, cert.slab_counted
...
theorem modifiedGreedyCanonicalWitnessDyadicShellOrException :
  ...
  intro witness
  exact Or.inl (modifiedGreedySmoothException_of_nonemptyWitness witness)
...
theorem modifiedGreedyCanonicalWitnessCancelledBlockProductsOrException :
  ...
  let hcanonical :
      CanonicalNonemptyRejectionWitnessData
        (modifiedGreedyAcceptedPrefixFromDeletion
          modifiedGreedyPrivateBarrierDeletionSet.Deleted) := by
    simpa [modifiedGreedyConstructionData] using witness.toCanonical
  exact Or.inl
    (Or.inr
      ⟨{ witness := hcanonical
         witness_endpoint := by
           dsimp [hcanonical, NonemptyRejectionWitnessData.toCanonical]
         dyadicShell := witness.n
         smooth_bound := True },
        trivial⟩)

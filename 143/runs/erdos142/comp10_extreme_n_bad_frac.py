#!/usr/bin/env python3
"""
comp10_extreme_n_bad_frac.py — Extend bad_frac + min_wfrac measurements to N=75000..100000
...
Prior results (cumulative):
  N=40000: avg_bad_frac=0.9719 avg_min_wfrac=0.2031 avg_witnesses=945
  N=50000: avg_bad_frac=0.9741 avg_min_wfrac=0.2000 avg_witnesses=1088

Schedule for this run:
  N=75000:  1 trial (2 if first finishes in <4 min)
  N=100000: 1 trial (if time allows)
"""
...
def analyze_trial(A_set, N):
    """Returns analysis dict, or None if A too small."""
    A_list = sorted(A_set)
    M = len(A_list)
    if M < 2:
        return None
...
def run_one_trial(N, trial_idx, restarts=2, verbose=True):
    """Run a single trial. Returns (result_dict, elapsed_seconds)."""
...
def summarise(results, N, elapsed_block):
    ...
    return dict(
        ...
        avg_witnesses=mean(n_wits),
        min_witnesses=min(n_wits),
        max_witnesses=max(n_wits),
        avg_max_popular=mean(max_pops),
        avg_M=mean(Ms),
...
def main():
    TOTAL_LIMIT = 1200.0   # 20-minute hard cap (N=100k can be slow)
    SECOND_TRIAL_THRESHOLD = 240.0  # add 2nd trial at N=75k if first < 4 min

    t_global = time.time()
    summaries = []

    print("comp10_extreme_n: bad_frac + min_wfrac at N=75000..100000", flush=True)
    print("=" * 120, flush=True)
    print("Prior: N=50000→bad_frac=0.9741, avg_min_wfrac=0.2000, witnesses=1088", flush=True)
    print("=" * 120, flush=True)

    # ── N = 75000 ─────────────────────────────────────────────────────────────
    N = 75000
    print(f"\n[N={N}] running trial 1...", flush=True)
    t_block = time.time()
    r1, t1 = run_one_trial(N, trial_idx=0)
    results_75k = [r1] if r1 is not None else []

    # Try a second trial if fast enough and time budget allows
    if t1 < SECOND_TRIAL_THRESHOLD and (time.time() - t_global) + t1 < TOTAL_LIMIT - 30:
        print(f"  [N={N}] trial 1 took {t1:.1f}s < {SECOND_TRIAL_THRESHOLD:.0f}s — running trial 2...",
              flush=True)
        r2, t2 = run_one_trial(N, trial_idx=1)
        if r2 is not None:
            results_75k.append(r2)

    s75 = summarise(results_75k, N, time.time() - t_block)
    if s75:
        summaries.append(s75)
        print_summary(s75)

    # ── N = 100000 ────────────────────────────────────────────────────────────
    N = 100000
    elapsed_so_far = time.time() - t_global
    if elapsed_so_far > TOTAL_LIMIT - 60:
        print(f"\n[N={N}] SKIPPED — time budget exhausted ({elapsed_so_far:.0f}s)", flush=True)
    else:
        print(f"\n[N={N}] running trial 1... (elapsed so far: {elapsed_so_far:.1f}s)", flush=True)
        t_block = time.time()
        r1, t1 = run_one_trial(N, trial_idx=0)
        results_100k = [r1] if r1 is not None else []

        # Try a second trial if fast enough
        if t1 < SECOND_TRIAL_THRESHOLD and (time.time() - t_global) + t1 < TOTAL_LIMIT - 30:
            print(f"  [N={N}] trial 1 took {t1:.1f}s — running trial 2...", flush=True)
            r2, t2 = run_one_trial(N, trial_idx=1)
            if r2 is not None:
                results_100k.append(r2)

        s100 = summarise(results_100k, N, time.time() - t_block)
        if s100:
